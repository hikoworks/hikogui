...
void gfx_pipeline_SDF::draw_in_command_buffer(vk::CommandBuffer commandBuffer, draw_context const& context)
{
...
std::vector<vk::PipelineShaderStageCreateInfo> gfx_pipeline_SDF::createShaderStages() const
{
    hi_axiom_not_null(device());
 ...
 */
std::vector<vk::PipelineColorBlendAttachmentState> gfx_pipeline_SDF::getPipelineColorBlendAttachmentStates() const
{
...
std::vector<vk::DescriptorSetLayoutBinding> gfx_pipeline_SDF::createDescriptorSetLayoutBindings() const
{
    return {
        {0, // binding
...
std::vector<vk::WriteDescriptorSet> gfx_pipeline_SDF::createWriteDescriptorSet() const
{
    hi_axiom_not_null(device());
...
size_t gfx_pipeline_SDF::getDescriptorSetVersion() const
{
    hi_axiom_not_null(device());
    return device()->SDF_pipeline->atlasTextures.size();
}

std::vector<vk::PushConstantRange> gfx_pipeline_SDF::createPushConstantRanges() const
{
    return push_constants::pushConstantRanges();
}

vk::VertexInputBindingDescription gfx_pipeline_SDF::createVertexInputBindingDescription() const
{
    return vertex::inputBindingDescription();
}

std::vector<vk::VertexInputAttributeDescription> gfx_pipeline_SDF::createVertexInputAttributeDescriptions() const
{
    return vertex::inputAttributeDescriptions();
}

void gfx_pipeline_SDF::build_vertex_buffers()
{
...
void gfx_pipeline_SDF::teardown_vertex_buffers()
{
    hi_axiom_not_null(device());
...
void gfx_pipeline_SDF::texture_map::transitionLayout(const gfx_device &device, vk::Format format, vk::ImageLayout nextLayout)
{
...
gfx_pipeline_SDF::device_shared::device_shared(gfx_device const& device) : device(device)
{
    buildShaders();
    buildAtlas();
}

gfx_pipeline_SDF::device_shared::~device_shared() {}

void gfx_pipeline_SDF::device_shared::destroy(gfx_device const *vulkanDevice)
{
...
[[nodiscard]] glyph_atlas_info gfx_pipeline_SDF::device_shared::allocate_rect(extent2 draw_extent, scale2 draw_scale) noexcept
{
...
void gfx_pipeline_SDF::device_shared::uploadStagingPixmapToAtlas(glyph_atlas_info const& location)
{
...
void gfx_pipeline_SDF::device_shared::prepareStagingPixmapForDrawing()
{
    stagingTexture.transitionLayout(device, vk::Format::eR8Snorm, vk::ImageLayout::eGeneral);
}

void gfx_pipeline_SDF::device_shared::prepare_atlas_for_rendering()
{
 ...
 *  O---------------------+
 */
void gfx_pipeline_SDF::device_shared::add_glyph_to_atlas(hi::font const &font, glyph_id glyph, glyph_atlas_info& info) noexcept
{
...
bool gfx_pipeline_SDF::device_shared::place_vertices(
...
void gfx_pipeline_SDF::device_shared::drawInCommandBuffer(vk::CommandBuffer const& commandBuffer)
{
    commandBuffer.bindIndexBuffer(device.quadIndexBuffer, 0, vk::IndexType::eUint16);
}

void gfx_pipeline_SDF::device_shared::buildShaders()
{
...
void gfx_pipeline_SDF::device_shared::teardownShaders(gfx_device const *vulkanDevice)
{
    hi_assert_not_null(vulkanDevice);
...
void gfx_pipeline_SDF::device_shared::addAtlasImage()
{
...
void gfx_pipeline_SDF::device_shared::buildAtlas()
{
...
void gfx_pipeline_SDF::device_shared::teardownAtlas(gfx_device const *vulkanDevice)
{
    hi_assert_not_null(vulkanDevice);
